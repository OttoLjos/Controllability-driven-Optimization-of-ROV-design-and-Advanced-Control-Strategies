...
# Define the duration for each test (e.g., 1.5 minutes)
...
# Function to calculate IAE
def calculate_iae(actual, setpoint, time):
    error = np.abs(actual - setpoint)
    iae = np.trapz(error, time)
    return iae

# Store IAEs for each controller
iae_results = {}
...
for controller in tested_controllers:
    ...
    # Calculate IAEs
    iae_depth = calculate_iae(df['Depth (m)'], df['Heave Setpoint'], elapsed_time)
    iae_roll = calculate_iae(df['Roll (radians)'], df['Roll Setpoint'], elapsed_time)
    iae_pitch = calculate_iae(df['Pitch (radians)'], df['Pitch Setpoint'], elapsed_time)

    iae_results[controller] = {
        'IAE Depth': iae_depth,
        'IAE Roll': iae_roll,
        'IAE Pitch': iae_pitch
    }
...
# Print IAE results
for controller, iaes in iae_results.items():
    print(f'{controller} Controller IAE:')
    for metric, iae_value in iaes.items():
        print(f'{metric}: {iae_value}')
    print()
